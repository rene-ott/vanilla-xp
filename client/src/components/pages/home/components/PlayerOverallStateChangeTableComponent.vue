<template>
  <table class="table table-bordered" :class="{ loading: isLoading }">
    <thead>
      <tr>
        <th scope="col" class="w-7">#</th>
        <th scope="col" class="w-15">Player</th>
        <th scope="col" class="w-12">XP Change</th>
        <th scope="col" class="w-12">XP Current</th>
        <th scope="col" class="w-12">XP Previous</th>

        <th scope="col" class="w-7">Level Change</th>
        <th scope="col" class="w-7">Level Current</th>
        <th scope="col" class="w-7">Level Previous</th>

        <th scope="col" class="w-7">Rank Change</th>
        <th scope="col" class="w-7">Rank Current</th>
        <th scope="col" class="w-7">Rank Previous</th>
      </tr>
    </thead>
    <tbody v-if="pageChanges.length > 0">
      <tr v-for="(item, index) in pageChanges" :key="item.playerName">
        ...
        <td>
          {{ item.notAvailable ? "N/A" : toLocaleNumber(item.rankChange) }}
          <template v-if="!item.notAvailable">
            <rank-arrow-component :rankChange="item.rankChange" />
          </template>
        </td>
        <td>{{ toLocaleNumber(item.rankCurrent) }}</td>
        <td>{{ item.notAvailable ? "N/A" : toLocaleNumber(item.rankPrevious) }}</td>
      </tr>
    </tbody>
    <tbody v-else>
      <tr v-for="n in pageSize" :key="n">
        <td :colspan="11" :class="{ 'bottom-border-removed': n != pageSize }">&nbsp;</td>
      </tr>
    </tbody>
  </table>
...
export default {
  ...
  emits: ["onDataLoading", "onDataLoaded"],
  ...
  methods: {
    ...
    handleRequestChangesResponse: function (response, vm) {
      vm.playersOverallChanges = response.data;
      this.isLoading = false;
      this.$emit("onDataLoaded");
    },

    loadChanges: function (vm) {
      this.playersOverallChanges = [];
      this.isLoading = true;
